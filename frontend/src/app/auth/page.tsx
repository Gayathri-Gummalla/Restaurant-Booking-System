...
import { Mail, Lock, User as UserIcon, ArrowLeft, ChevronRight } from 'lucide-react';
...
export default function AuthPage() {
    ...
    const [socialPicker, setSocialPicker] = useState<{ isOpen: boolean, provider: string }>({ isOpen: false, provider: '' });
    ...
    const validateForm = () => {
        ...
        // Removed confirmPassword check to shorten the process
        setErrors(newErrors);
        return isValid;
    };
    ...
    const handleSocialLogin = (provider: string) => {
        setSocialPicker({ isOpen: true, provider });
    };

    const confirmSocialLogin = async (accountName: string, email: string) => {
        setSocialPicker({ ...socialPicker, isOpen: false });
        setLoading(true);
        try {
            ...
            const mockToken = 'social-mock-token-' + Math.random().toString(36).substr(2);

            addToast('success', `Signed in with ${socialPicker.provider}!`, `Welcome back, ${accountName}.`);
            ...
            addToast('error', 'Authentication Failed', 'Please try again.');
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="relative min-h-screen w-full flex items-center justify-center p-6 overflow-hidden">
            ...
            {socialPicker.isOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
                    <div className="absolute inset-0 bg-black/60 backdrop-blur-md" onClick={() => setSocialPicker({ ...socialPicker, isOpen: false })} />
                    <div className="bg-white w-full max-w-sm rounded-[2.5rem] shadow-3xl overflow-hidden relative animate-in fade-in zoom-in duration-300">
                        <div className="p-8 text-center border-b border-gray-50">
                            <div className="flex justify-center mb-6">
                                {socialPicker.provider === 'Google' ? (
                            ...
                            <h3 className="text-xl font-bold text-gray-900 mb-2">Choose an account</h3>
                            <p className="text-xs text-gray-500 font-medium">to continue to <span className="text-gold font-bold">Spice Garden</span></p>
                        </div>

                        <div className="p-4 space-y-2 max-h-[400px] overflow-y-auto">
                            {[
                                { name: 'John Doe', email: 'john.doe@gmail.com', initial: 'J', color: 'bg-blue-500' },
                                { name: 'Jane Smith', email: 'jane.smith@outlook.com', initial: 'J', color: 'bg-purple-500' },
                                { name: 'Gayathri G', email: 'gayathri.g@spicegarden.com', initial: 'G', color: 'bg-gold' }
                            ].map((acc, idx) => (
                                <button
                                    key={idx}
                                    onClick={() => confirmSocialLogin(acc.name, acc.email)}
                                ...
                                >
                                    ...
                                    <ChevronRight size={16} className="text-gray-300 opacity-0 group-hover:opacity-100 transition-all -translate-x-2 group-hover:translate-x-0" />
                                </button>
                            ))}

                            <button className="w-full flex items-center gap-4 p-4 rounded-2xl hover:bg-gray-50 transition-all text-left mt-2 border border-transparent">
                        ...
                        <div className="p-6 bg-gray-50/50 text-center">
                            <p className="text-[9px] text-gray-400 uppercase tracking-widest font-bold leading-relaxed px-4">
                                By continuing, {socialPicker.provider} will share your name, email address, language preference, and profile picture with Spice Garden.
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-[2rem] shadow-[0_50px_100px_rgba(0,0,0,0.3)] p-8 md:p-12 relative overflow-hidden group">
                    ...
                    <div className="text-center mb-10">
                        ...
                        <p className="text-[10px] text-gold font-bold uppercase tracking-[0.4em] mb-3">
                            {isLogin ? 'Welcome Back' : 'Fast 1-Step Registration'}
                        </p>
                    ...
                    <form className="space-y-5" onSubmit={handleSubmit}>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <button
                                ...
                                disabled={loading}
                                className="flex items-center justify-center gap-3 py-4 px-4 bg-white border border-gray-100 rounded-xl hover:border-gold/30 hover:bg-gray-50 transition-all group active:scale-95 shadow-sm hover:shadow-md disabled:opacity-50"
                            >
                                ...
                                <span className="text-[10px] font-black uppercase tracking-widest text-gray-900">Google</span>
                            </button>
                            <button
                                type="button"
                            ...
                            >
                                ...
                                <span className="text-[10px] font-black uppercase tracking-widest text-gray-900">Microsoft</span>
                            </button>
                        </div>
                    </form>
